...
class ViewHelperResolver extends \TYPO3Fluid\Fluid\Core\ViewHelper\ViewHelperResolver
{
    ...
    {
        static $instances = [];
        if (!isset($instances[$viewHelperClassName])) {
            $instances[$viewHelperClassName] = $this->getObjectManager()->get($viewHelperClassName);
        }
        return clone $instances[$viewHelperClassName];
    }
