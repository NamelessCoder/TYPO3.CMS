...
class CategoryRegistryTest extends \TYPO3\TestingFramework\Core\Unit\UnitTestCase
{
    ...
    public function tabIsAddedOnlyOncePerTable()
    {
        ...
        foreach ($GLOBALS['TCA'][$this->tables['first']]['types'] as $typeConfig) {
            $this->assertSame(
                1, substr_count($typeConfig['showitem'], '--div--;LLL:EXT:core/Resources/Private/Language/locallang_tca.xlf:sys_category.tabs.category')
            );
        }
    }
    ...
    public function addInitializesMissingTypes()
    {
        ...
        $this->assertSame(
            1, substr_count($GLOBALS['TCA'][$this->tables['first']]['types']['newtypeafterfirstadd']['showitem'], '--div--;LLL:EXT:core/Resources/Private/Language/locallang_tca.xlf:sys_category.tabs.category')
        );
    }
